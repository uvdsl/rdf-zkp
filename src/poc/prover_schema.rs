...
use sophia::{
    ...
    inmem::{dataset::LightDataset, graph::LightGraph, index::SimpleTermIndex},
    iri::Iri,
    turtle::{
        parser::trig,
        serializer::{
...
fn load_from_issuer() -> (
    ...
    let msgs = rdf2fr_encoder
        .quads_to_field_representations(&canon_cred_dataset)
        .unwrap();
...
pub fn prove() {
    ...
    let indices_to_hide = vec![0, 2, 4, 5, 6]; // * from verifier (derived) // remember: hide blank nodes in object position
    let indices_to_reveal = vec![1, 3, 7]; // * from verifier (derived)
    ...
    let eqiv = |i| match i {
        ...
        // ! not sure if this must stay like this? because of RDF graph...!
        0 | 5 | 10 | 15 | 20 => 0,
        7 | 25 | 30 => 2, // cannot hide relation between credentialSubject and headOf, i.e. node 25
        17 => 4,
        22 => 5,
        27 => 6,
        4 | 9 | 14 | 19 | 24 | 29 | 34 => usize::max_value(), // not nice but works for now ... could amend usize to str to give names but thats future refractoring
        _ => i,
    };
    let indices_to_hide_rdf = vec![
        0, 4, 5, 7, 8, 9, 10, 14, 15, 17, 18, 19, 20, 22, 23, 24, 25, 27, 28, 29, 30,
        34, // remember not to hide predicates... e.g. 16, 26
    ];
    let mut presentation_quads_list =
        derive_presentation(indices_to_hide_rdf, &eqiv, &cred_dataset).unwrap();
    // serializing the graph

    let mut graph = LightGraph::new(); // unordered
    let eqiv_none = |i: usize| i;
    stmt_proofs_to_rdf(
        stmts_and_proofs,
        indices_to_hide,
        &eqiv_none,
        &BnodeId::new_unchecked(usize::max_value().to_string().as_str())
            .into_term::<SimpleTerm<'_>>(),
    ...
    // This is just ugly because the sophia trig serialisers will not preserve ordering of quads.
    let presentation = serialize_ordered_trig(presentation_quads_list);
    // println!("{}", presentation);

    // Open a file to write the serialized graph
    let mut file = File::create("./data/prover_schema.trig").unwrap();
